...
- **Durable history (tiered)**: every change batch is appended to `~/.local/share/quest-journal/history.jsonl` (`HistoryStore`, one `HistoryEntry` per batch). Retention is *month-based, not time-based*: the `DetailMonths` (4) most recent calendar months stay as raw detail; when a batch lands in a 5th distinct month, the oldest month is compacted (`MonthCompactor`) into `history-archive.jsonl` (`HistoryArchiveStore`, one `HistoryArchiveMonth` carrying `TotalXp`, `AverageXpPerDay`, and final `ArchivedQuestState`s) and dropped from the detail log. All bucketing is by **local** calendar date/month. `quest history --graph [--week|--month|--year|--all]` renders two stacked Spectre `BarChart`s per the chosen scope — XP per bucket, then completed-task count per bucket; `XpHistoryGraph.Build(..., GraphMetric)` does the (pure, testable) aggregation for either metric, merging archive months (`TotalXp`/`CompletedCount`) for the `--year`/`--all` scopes. The archive stores no journal path, so monthly scopes are journal-agnostic (v1 trade-off).
